  ...
  template: `
    <div class="container-fluid py-4">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <h2 class="fw-bold text-hotel mt-2"><i class="bi bi-tags-fill me-2"></i>Gestión de Tipos de Habitación</h2>
        </div>
        <button class="btn btn-hotel" (click)="openCreateModal()"><i class="bi bi-plus-lg me-1"></i> Nuevo Tipo</button>
      </div>
      @if (toast.show) {
      ...
      @if (!loading) {
        <div class="card shadow-sm"><div class="table-responsive"><table class="table table-hover mb-0">
          <thead class="table-light"><tr><th>ID</th><th>Nombre</th><th>Descripción</th><th>Estado</th><th class="text-end">Acciones</th></tr></thead>
          <tbody>
            @for (item of itemsPaginados; track item.idTipoHabitacion) {
              <tr>
                <td class="fw-bold">{{item.idTipoHabitacion}}</td>
                <td>{{item.nombreTipoHabitacion}}</td>
                <td>{{item.descripcionTipoHabitacion || '-'}}</td>
                <td><span class="badge" [class.bg-success]="item.estadoTipoHabitacion" [class.bg-danger]="!item.estadoTipoHabitacion">{{item.estadoTipoHabitacion ? 'Activo' : 'Inactivo'}}</span></td>
                <td class="text-end">
            ...
            } @empty { <tr><td colspan="5" class="text-center text-muted py-4">No hay datos</td></tr> }
          </tbody>
        </table></div>
        @if (totalPages > 1) {
    ...
    @if (showModal) {
      <div class="modal fade show d-block" style="background:rgba(0,0,0,0.5)"><div class="modal-dialog"><div class="modal-content">
        <div class="modal-header bg-hotel text-white"><h5 class="modal-title">{{isEditing?'Editar':'Nuevo'}} Tipo</h5>
          <button type="button" class="btn-close btn-close-white" (click)="closeModal()"></button></div>
        <div class="modal-body">
          <div class="mb-3"><label class="form-label">Nombre *</label><input type="text" class="form-control" [(ngModel)]="formData.nombreTipoHabitacion"></div>
          <div class="mb-3"><label class="form-label">Descripción</label><textarea class="form-control" [(ngModel)]="formData.descripcionTipoHabitacion"></textarea></div>
          <div class="form-check"><input type="checkbox" class="form-check-input" [(ngModel)]="formData.estadoTipoHabitacion"><label class="form-check-label">Activo</label></div>
        </div>
    ...
    @if (showDeleteModal) {
      <div class="modal fade show d-block" style="background:rgba(0,0,0,0.5)"><div class="modal-dialog"><div class="modal-content">
        <div class="modal-header bg-danger text-white"><h5 class="modal-title">Eliminar Tipo</h5></div>
        <div class="modal-body"><p>¿Eliminar <strong>{{selectedItem?.nombreTipoHabitacion}}</strong>?</p></div>
        <div class="modal-footer">
...
export class TiposHabitacionAdminComponent implements OnInit {
  ...
  items: TipoHabitacion[] = []; filteredData: TipoHabitacion[] = [];
  loading = true; saving = false;
  searchTerm = ''; filterStatus = ''; page = 1; perPage = 20;
  showModal = false; showDeleteModal = false; isEditing = false; selectedItem: TipoHabitacion | null = null;
  formData = { idTipoHabitacion: 0, nombreTipoHabitacion: '', descripcionTipoHabitacion: '', estadoTipoHabitacion: true };
  ...
  loadData() {
    this.loading = true;
    this.http.get<TipoHabitacion[]>(`${this.apiUrl}/TiposHabitacion`).subscribe({
  ...
  filterData() {
    ...
    if (this.searchTerm) result = result.filter(i => i.nombreTipoHabitacion?.toLowerCase().includes(this.searchTerm.toLowerCase()));
    if (this.filterStatus !== '') result = result.filter(i => i.estadoTipoHabitacion === (this.filterStatus === 'true'));
  ...
  openCreateModal() { this.isEditing = false; this.formData = { idTipoHabitacion: 0, nombreTipoHabitacion: '', descripcionTipoHabitacion: '', estadoTipoHabitacion: true }; this.showModal = true; }
  openEditModal(i: TipoHabitacion) { this.isEditing = true; this.selectedItem = i; this.formData = { ...i }; this.showModal = true; }
  openDeleteModal(i: TipoHabitacion) { this.selectedItem = i; this.showDeleteModal = true; }
  closeModal() { this.showModal = false; }

  saveItem() {
    if (!this.formData.nombreTipoHabitacion) { this.showToast('Nombre requerido', 'error'); return; }
    this.saving = true;
    const url = this.isEditing ? `${this.apiUrl}/TiposHabitacion/${this.formData.idTipoHabitacion}` : `${this.apiUrl}/TiposHabitacion`;
    (this.isEditing ? this.http.put(url, this.formData, { headers: this.getAuthHeaders() }) : this.http.post(url, this.formData, { headers: this.getAuthHeaders() })).subscribe({
      ...
      error: () => { this.showToast('Error (requiere autenticación)', 'error'); this.saving = false; }
    });
  }

  deleteItem() {
    if (!this.selectedItem) return;
    this.http.delete(`${this.apiUrl}/TiposHabitacion/${this.selectedItem.idTipoHabitacion}`, { headers: this.getAuthHeaders() }).subscribe({
